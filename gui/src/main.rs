...
fn App() -> Element {
    ...
    let mut cli_version = use_signal(|| String::from("..."));

    use_future(move || async move {
        if let Ok(output) = tokio::process::Command::new(which_devbind())
            .arg("-v")
            .output()
            .await
        {
            ...
            if let Some(v) = s.split_whitespace().nth(1) {
                cli_version.set(v.to_string());
            } else {
                cli_version.set(String::from("unknown"));
            }
        }
    });
    ...
    rsx! {
        ...
        div { class: "flex h-screen overflow-hidden",
            ...
            aside { class: "w-64 sidebar flex flex-col z-10",
                div { class: "p-8 mb-4",
                    h1 { class: "text-lg font-bold tracking-tighter mono mx-auto flex items-center gap-2",
            ...
            main { class: "flex-1 flex flex-col",
                ...
                div { class: "p-10 flex-1 overflow-y-auto flex flex-col",
                    div { class: "flex-1",
                        if dashboard_active {
                            MappingsTab {
                                config: config,
