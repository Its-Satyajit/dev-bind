...
/// Build a dummy allowed domains set for testing.
fn dummy_allowed() -> HashSet<String> {
    let mut set = HashSet::new();
    set.insert("myapp.test".to_string());
    set.insert("mirror.test".to_string());
    set.insert("a.b.c.d.test".to_string());
...
fn test_a_query_for_test_domain() {
    ...
    let resp = handle_query(&query, &dummy_allowed());
...
fn test_truncated_packet_returns_formerr() {
    ...
    let resp = handle_query(&short_pkt, &dummy_allowed());
